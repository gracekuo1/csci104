...
Board::Board(const Board& b) :
    vehicles_(b.vehicles_), grid_(b.grid_), past_boards(b.past_boards)
{
}
...
bool Board::move(VID_T vid, int amount)
{
    if(isLegalMove(vid, amount)) {
        ...
        updateBoard();
        MovePair temp = make_pair(vid, amount);
        past_boards.push(temp);
        return true;
    }
    else {
        return false;
...
{
    MovePair undo = past_boards.top();

    /*if (past_boards.empty()){
        throw std::underflow_error("no boards to undo");
    }*/
    
    Vehicle& temp = vehicles_[undo.first];
    move(temp.id, -1 * undo.second);
    past_boards.pop();
    past_boards.pop();
    updateBoard();
...
Board::MovePairList Board::potentialMoves() const
{
    ...
    Board::MovePairList total_moves;
    map<char, Vehicle>::const_iterator it_car = vehicles_.begin();
    
    while(it_car != vehicles_.end()){
        int amount_move = 1;
        while (isLegalMove(it_car->first, amount_move)){
            MovePair p (it_car->first, amount_move);
            std::cout << it_car->first << " " << amount_move << endl;
...
bool Board::operator<(const Board& other) const
{
    ...
    // To avoid compile errors in skeletong - Replace this
    //return false
    VehicleGrid current_b = this->grid_;
    VehicleGrid other_b = other.grid_;

    int current_total_row = current_b.size();
    int current_total_col = current_b.size();

    for (int r = 0; r < current_total_row; r++){
        for (int c = 0; c < current_total_col; c++){
            // VID_T current_b = this->at(r, c);
            // VID_T other_b = other.at(r, c);

            // if (other_b < current_b){
            //     return true;
            // }
            // if (other_b > current_b){
            //     return false;
