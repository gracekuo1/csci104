...
public:
    ...
    HASH_INDEX_T next() 
    {
        ...
        //multiple by numProbes because all values dont change except for numProbes (0, 1, 2, 3)
...
private:
    ...
    //fishy add data member that keeps track of size
    size_t size_;
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    mIndex_ = 0;
    table_.resize(CAPACITIES[mIndex_]); //inital size of table
    size_ = 0;
    internalSize_ = 0;
    alpha_ = resizeAlpha;
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if ((double)internalSize_/table_.size() >= alpha_){
        this->resize();
    }
    HASH_INDEX_T place = this->probe(p.first);
    //if index not found
    if (place == npos){
    ...
    //if it's deleted
    if(table_[place] != nullptr){
        delete table_[place];
    }
    else {
        //need to handle deleted flag fishy
        size_++;
        internalSize_++;
    }

    //place it in
    HashItem* newHashItem = new HashItem (p);
    table_[place] = newHashItem;
}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> copyTable = table_;

    table_.clear();
    table_.resize(newSize);
    internalSize_ = size_;

    for (size_t i = 0; i < copyTable.size(); i ++){
        if (copyTable[i]!= nullptr && copyTable[i]->deleted != true){
            HASH_INDEX_T place = this->probe(copyTable[i]->item.first);
            table_[place] = copyTable[i];
        }
    }
}
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        // angle bracket get the 0th thing out of a pair FISHY
        else if(this->kequal_(key, table_[loc]->item.first) && !table_[loc]->deleted) { ///FILL IN LATER FISHY
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
